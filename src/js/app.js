...
$(function() {
  $(document).on('mouseup', function(e) {
    if($(e.target).closest('.top-line__country-href').is('.top-line__country-href'))
    {
    ...
    else if(!$(e.target).closest('.top-line__country-list').is('.top-line__country-list'))
    {
...
// navigation
$(function() {
  $('.navigation__submenu').siblings('.navigation__link').addClass('navigation__slink');

  //var $menu = $('.navigation');
  
  $(document).on('click', function(e) {
    if($(e.target).closest('.navigation__slink').is('.navigation__slink'))
    {
      e.preventDefault();
      $(e.target).siblings('.navigation__submenu').toggleClass('is-opened');
    ...
    else if(!$(e.target).closest('.navigation__submenu').is('.navigation__submenu'))
    {
...
$(window).on('resize load', function() {
  if($(this).outerWidth() <= 991) {
    $('.top-line').sticky({topSpacing:0,className:'navigation__shadow',zIndex:'11'});
    $('.navigation').unstick();
  } else {
    $('.navigation').sticky({topSpacing:0,className:'navigation__shadow',zIndex:'10'});
    $('.top-line').unstick();
  }
...
$(document).on('click touchend','.js--tab',function(e) {
  var tabsItem = $(this).parent().find('.tabs__item');
...
// project tabs
$('.inp__item').on('click touchend',function() {
